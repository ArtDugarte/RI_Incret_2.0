...
public class Listado_estuXprofe extends javax.swing.JInternalFrame {

    public Listado_estuXprofe(int id) {
    ...
    private void initComponents() {
        ...
        TablaEstudiante.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Nombre", "Apellido", "Cédula", "Dirección", "Número Telefónico", "Progreso"
            }
        ) {
    ...
    public void obtenerMatrizEstudiante() {
        ...
        ArrayList<Modelo> miLista = estudiante.BuscarconMatriz(id_usuario);
        DefaultTableModel model = (DefaultTableModel) TablaEstudiante.getModel();

        model.setRowCount(0);
        for (int i = 0; i < miLista.size(); i++) {
            
            model.addRow(new Object[]{miLista.get(i).getNombre(), miLista.get(i).getApellido(), miLista.get(i).getcedula(), miLista.get(i).getDirec(), miLista.get(i).getNumTlf(), definir_progreso(miLista.get(i).getProgreso())});
        }
    }
    
    public String definir_progreso(int p){
    
        if(p == 0){
            
            progreso = "SIN ASIGNAR";
        }
        if(p == 1){
    ...
    private String usuario, cedula_estudiante = "", serial_i = "", progreso = "";
